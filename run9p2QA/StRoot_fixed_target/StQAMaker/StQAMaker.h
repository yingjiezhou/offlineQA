...
public:
  StQAMaker(const Char_t *name , StPicoDstMaker *picoMaker);
  ~StQAMaker();
...
protected:
  void     printConfig();
  void     bookHistos();
  Bool_t   processPicoDstEvent();
  Bool_t   isValidTrack(StPicoTrack *pPicoTrack) const;
  Bool_t   getBemcInfo(StPicoTrack *pPicoTrack, const Int_t runIdx, Int_t &nMthTrks, Int_t &nTrigTrks, Int_t &nBEMCeCans);
  Bool_t   getMtdInfo(StPicoTrack *pPicoTrack, const Int_t runIdx);
...
private:
  ...
  static const Int_t    mTotalRuns =  20;
  ...
  TProfile *hgRefMultvsRun;
  TProfile *hTofMultvsRun;
  TProfile *hnTofMatchvsRun;
  TProfile *hnBEMCMatchvsRun;
  ...
  // MTD match trakcs
  TProfile *hMthTrkDeltaYvsRun;
  TProfile *hMthTrkDeltaZvsRun;
  TProfile *hMthTrkDeltaTOFvsRun;
