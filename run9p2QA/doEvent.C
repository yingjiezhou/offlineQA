...
void doEvent(Int_t nEvents=-1, const Char_t *inputFile="test.list", const TString outputFile="test/test.root", const Bool_t debug = kFALSE)
{
  ...
  StPicoDstMaker* picoDstMaker = new StPicoDstMaker(2, inputFile, "picoDstMaker");
	...
	qaMaker->setMaxVtxR(6.);
//	qaMaker->setMaxVtxZ(100.);// default
  qaMaker->setMaxVtxZ(800.);
	qaMaker->setMaxVzDiff(1e3);
//	qaMaker->setMinTrackPt(0.2); // default
  qaMaker->setMinTrackPt(0.15);

	qaMaker->setMaxTrackEta(150); // not used
...
//  qaMaker->setMaxDca(1.5);
  ...
  IntVec triggers;
  triggers.push_back(610001);
  triggers.push_back(610011);
  triggers.push_back(610021);
  triggers.push_back(610031);
  triggers.push_back(610041);
  triggers.push_back(610051);
  qaMaker->setTriggerID(triggers);

  //std::array<unsigned int, 6> const triggers = {610001,610011,610021,610031,610041,610051};
//  qaMaker->setTriggerID(triggers);
