...
Int_t runStart[nTrgIds] ={21246012}; // run20 21198005
Int_t runStop[nTrgIds] = {21251016};//21213020, 21237038
...
void plotQA_offline_run20_7p7(){
  ...
  TFile *f = new TFile( "rootfile/0911_run20_7p7_v3.root" );
  if(!f->IsOpen()){
  ...
  TString dir = "plots_0911_run20_7p7";
  TString badRunDir = "plots_0911_run20_7p7/badruns";

  system(Form("mkdir -p %s", dir.Data()));
  system(Form("rm -rf %s/*", dir.Data()));
  system(Form("mkdir -p %s/badruns", dir.Data()));
  system(Form("rm -rf %s/badruns/*", dir.Data()));
  ...
  TPDF *ps = new TPDF(Form("%s/Run20.7p7.QA.pdf", dir.Data()),111);
...
//  drawLatex(0.20, 0.48, "Default Vertex", 22, 0.06, 1);
  ...
  */
  ...
  if(setProfileRange) hZdcXvsRun->SetAxisRange(-0.1, 0.1, "Y");
  hZdcXvsRun->GetYaxis()->SetMaxDigits(2);
  hZdcXvsRun->Draw("p");
  for(Int_t i=0; i<nTrgIds; i++){
  ...
  hZdcXoverBbcXvsRun->GetYaxis()->SetMaxDigits(2);
