...
//Int_t runStart[nTrgIds] ={21030030}; // run20
//Int_t runStop[nTrgIds] = {21213020};

Int_t runStart[nTrgIds] ={20057003};
Int_t runStop[nTrgIds] = {20093036};
...
void plotQA_offline_run19_19p6(){
  
  cout<<"Start to draw the QA plots ..."<<endl;
  gStyle->SetOptFit(1111);
  gStyle->SetOptTitle(1);
  
  TFile *f = new TFile( "rootfile/0809_run19_19p6_MTD.root" );
  if(!f->IsOpen()){
  ...
  TString dir = "plots_0809_run19_19p6_MTD";
  system(Form("mkdir -p %s", dir.Data()));
  system(Form("rm -rf %s/*", dir.Data()));
  
  TString badRunDir = "plots_0809_run19_19p6_MTD/badruns";
...
//  TPDF *ps = new TPDF(Form("%s/Run20.9p2.QA.pdf", dir.Data()),111);
  TPDF *ps = new TPDF(Form("%s/Run19.19p6.QA.pdf", dir.Data()),111);
...
//  c1->SaveAs(Form("%s/refMultRatio_selVtxOverdelVtx.png", dir.Data()));
  ...
  TH2F *mhMtdHitMap  = (TH2F *)f->Get("mhMtdHitMap");
  TH2F *mhMtdMthHitMap  = (TH2F *)f->Get("mhMtdMthHitMap");
...
//  c1->SaveAs(Form("%s/phivsPt_FF.png", dir.Data()));
  ...
  c1->cd();
  mhMtdHitMap->Draw("colz");
  pdfAction(c1, ps);
  c1->SaveAs(Form("%s/mtdHit.png", dir.Data()));
  
  c1->cd();
  mhMtdMthHitMap->Draw("colz");
  pdfAction(c1, ps);
  c1->SaveAs(Form("%s/mthMtdHit.png", dir.Data()));
...
//  c1->SaveAs(Form("%s/trigBEMCe_etavsPhi.png", dir.Data()));
  ...
  hnEvtsvsRun                 = (TProfile *)f->Get("hnEvtsvsRun");
  ...
  TProfile *mhNMtdHitsVsRun     = (TProfile *)f->Get("mhNMtdHitsVsRun");
  TProfile *mhNMtdMthHitsVsRun     = (TProfile *)f->Get("mhNMtdMthHitsVsRun");
  ...
  hnEvtsvsRun->Draw("p");
  pdfAction(c1, ps);
  c1->SaveAs(Form("%s/nEvtsvsRun.png", dir.Data()));
  ...
  c1->cd();
  setProfile(mhNMtdHitsVsRun, MStyle, MSize, MColor, MColor);
  if(setProfileRange) mhNMtdHitsVsRun->SetAxisRange(2, 6, "Y");
  mhNMtdHitsVsRun->Draw("p");
//  for(Int_t i=0; i<nTrgIds; i++){
//    addShade(mhNMtdHitsVsRun, binLow[i], binHi[i], i);
//  }
//  dumpBadRuns(badRunDir, "mhNMtdHitsVsRun", varIdx);
//  pdfAction(c1, ps);
  c1->SaveAs(Form("%s/mhNMtdHitsVsRun.png", dir.Data()));
  
  c1->cd();
  setProfile(mhNMtdMthHitsVsRun, MStyle, MSize, MColor, MColor);
  if(setProfileRange) mhNMtdMthHitsVsRun->SetAxisRange(0, 1., "Y");
  mhNMtdMthHitsVsRun->Draw("p");
//  for(Int_t i=0; i<nTrgIds; i++){
//    addShade(mhNMtdMthHitsVsRun, binLow[i], binHi[i], i);
//  }
//  dumpBadRuns(badRunDir, "mhNMtdMthHitsVsRun", varIdx);
//  pdfAction(c1, ps);
  c1->SaveAs(Form("%s/mhNMtdMthHitsVsRun.png", dir.Data()));
...
//  indata.open("run20_9p2_stPhysics_runnumber_DD.dat");
  indata.open("rootfile/0807_run19_19p6_stPhysics_runnumber_DD.dat");
