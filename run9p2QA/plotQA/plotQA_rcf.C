...
Int_t runStart[nTrgIds] ={21030030};
Int_t runStop[nTrgIds] = {21190055};
...
void plotQA(){
  ...
  TFile *f = new TFile( "run20_9p2_all.root" );
  if(!f->IsOpen()){
...
////  c1->SaveAs(Form("%s/nHT2EvtsvsRun.png", dir.Data()));
  ...
  if(setProfileRange) hNTrksvsRun->SetAxisRange(60, 140, "Y");
  hNTrksvsRun->Draw("p");
  for(Int_t i=0; i<nTrgIds; i++){
...
Bool_t Init()
{
  ...
  indata.open("../run20_stPhysics_runnumber_DD.dat");
  mRunId2RunIdx.clear();
  mRunIdx2RunId.clear();
  if(indata.is_open()){
