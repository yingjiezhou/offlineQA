...
void pionYieldRuns_production(TString dir, int runStart, int runEnd, float energy, TString eventConfig, TString outputDir, Bool_t eToF_req){
 ...
 if (dEdxSliceEta->GetMaximum() == 0){
  ...
  double norm = 1.0/(nEventsVar.at(run_count)) * 1.0/(2*TMath::Pi()) * 1.0/(0.475) * 1.0/0.5 * 1.0/0.5 * 1.0/dEdxSliceEta->GetBinWidth(2);
...
if (dEdxSliceY->GetMaximum() == 0){
  ...
  gStyle->SetOptFit(1);
  f2->Draw("same");
  double normY = 1.0/(nEventsVar.at(run_count)) * 1.0/(2*TMath::Pi()) * 1.0/(0.475) * 1.0/0.5 * 1.0/0.5 * 1.0/dEdxSliceY->GetBinWidth(2);
...
yieldsPiY->GetYaxis()->SetTitleOffset(1.0);
 yieldsPiY->GetYaxis()->SetRangeUser(0,10.0);
...
yieldsPi->GetYaxis()->SetTitleOffset(1.0);
 yieldsPi->GetYaxis()->SetRangeUser(0,10.0);
