...
Int_t runStart[nTrgIds] ={12126101};
Int_t runStop[nTrgIds] = {12171017}; 
//const Int_t nTrgIds = 2;
//Int_t runStart[nTrgIds] ={12126101, 12153002};//for 2 different ht2 trigerIDs, the magnet polarity changed since 12153002
//Int_t runStop[nTrgIds] = {12152016, 12171017}; 
...
void plotQA(){
	...
	TFile *f = new TFile( "run11.ht2.QA.root" );
	if(!f->IsOpen()){
	...
	TString badRunDir = "plots/badruns";
	...
	TPDF *ps = new TPDF(Form("%s/run11.HT2.QA.pdf", dir.Data()),111);
	ps->Off();
	...
	TLegend *leg = new TLegend(0.56, 0.8, 0.86, 0.9);
	...
	leg->AddEntry(hEvent, "Selected vertex", "l");
	leg->Draw("same");
	...
	drawLatex(0.165, 0.30, Form("+ V_{r}<%1.1f cm: %1.2f%%", mVrCut, hEvent->GetBinContent(9)*1./hEvent->GetBinContent(3)*100), 22, 0.045, 4);
	drawLatex(0.165, 0.25, Form("+ |TPC V_{z}|<%1.1f cm: %1.2f%%", mVzCut, hEvent->GetBinContent(10)*1./hEvent->GetBinContent(3)*100), 22, 0.045, 4);
	drawLatex(0.165, 0.20, Form("+ |TPC V_{z} - VPD V_{z}|<%1.1f cm: %1.2f%%", mVzDiffCut, hEvent->GetBinContent(11)*1./hEvent->GetBinContent(3)*100), 22, 0.045, 4);
	pdfAction(c1, ps);
	...
	//drawLine(-mVzDiffCut, 0, -mVzDiffCut, hVzDiff_DefVtx->GetMaximum()*0.05, 2, 2, 2);
	//drawLine( mVzDiffCut, 0,  mVzDiffCut, hVzDiff_DefVtx->GetMaximum()*0.05, 2, 2, 2);
	...
	//setLegend(leg, 0.16, 0.2, 0.5, 0.36, 0.05);
	...
	hnHT2EvtsvsRun                 = (TProfile *)f->Get("hnHT2EvtsvsRun");
	...
	c1->cd();
	setHisto(hnHT2EvtsvsRun, MStyle, MSize, MColor, MColor);
	hnHT2EvtsvsRun->Draw("p");
	pdfAction(c1, ps);
	c1->SaveAs(Form("%s/nHT2EvtsvsRun.png", dir.Data()));
	...
	if(dumpEmptyHT2Runs){
		...
		for(Int_t i=grabRunIdx(runStart[0]); i<=grabRunIdx(runStop[nTrgIds-1]); i++){
			if(hnHT2EvtsvsRun->GetBinContent(i+1)==0){
	...
	if(setProfileRange) hZdcXvsRun->SetAxisRange(0, 80, "Y");
	hZdcXvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hBbcXvsRun->SetAxisRange(0, 80, "Y");
	hBbcXvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hZdcXoverBbcXvsRun->SetAxisRange(0.4, 1.2, "Y");
	...
	if(setProfileRange) hTpcVxvsRun->SetAxisRange(0.1, 0.4, "Y");
	hTpcVxvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hTpcVyvsRun->SetAxisRange(-0.15, 0.15, "Y");
	hTpcVyvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hTpcVzvsRun->SetAxisRange(-20, 10, "Y");
	hTpcVzvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hVpdVzvsRun->SetAxisRange(-20, 10, "Y");
	hVpdVzvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hVzDiffvsRun->SetAxisRange(-1.5, 1.5, "Y");
	hVzDiffvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hRefMultvsRun->SetAxisRange(240, 400, "Y");
	...
	if(setProfileRange) hNTrksvsRun->SetAxisRange(400, 560, "Y");
	hNTrksvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hPtvsRun->SetAxisRange(0.6, 0.7, "Y");
	hPtvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hEtavsRun->SetAxisRange(-0.02, 0.03, "Y");
	hEtavsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hPhivsRun->SetAxisRange(-0.05, 0.1, "Y");
	hPhivsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hDcavsRun->SetAxisRange(0.2, 1.2, "Y");
	hDcavsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hNHitsFitvsRun->SetAxisRange(29, 32, "Y");
	...
	if(setProfileRange) hNHitsPossvsRun->SetAxisRange(39, 41, "Y");
	...
	if(setProfileRange) hNHitsDedxvsRun->SetAxisRange(18, 20, "Y");
	...
	if(setProfileRange) hDedxvsRun->SetAxisRange(2.5, 2.9, "Y");
	hDedxvsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hNSigmaEvsRun->SetAxisRange(-4.8, -3.6, "Y");
	...
	if(setProfileRange) hBetavsRun->SetAxisRange(0.8, 1.8, "Y");
	hBetavsRun->Draw("p");
	for(Int_t i=0; i<nTrgIds; i++){
	...
	if(setProfileRange) hNMthTrksvsRun->SetAxisRange(60, 160, "Y");
	...
	if(setProfileRange) hMthTrkNPhivsRun->SetAxisRange(0.6, 1.0, "Y");
	...
	if(setProfileRange) hMthTrkEtavsRun->SetAxisRange(-0.1, 0.1, "Y");
	...
	if(setProfileRange) hMthTrkPhivsRun->SetAxisRange(-0.3, 0.1, "Y");
	...
	if(setProfileRange) hMthTrkNSigmaEvsRun->SetAxisRange(-4.8, -3.6, "Y");
	...
	if(setProfileRange) hMthTrkAdc0vsRun->SetAxisRange(90, 100, "Y");
	...
	if(setProfileRange) hMthTrkEvsRun->SetAxisRange(2.05, 2.15, "Y");
	...
	if(setProfileRange) hMthTrkZDistvsRun->SetAxisRange(-0.20, 0.05, "Y");
	...
	if(setProfileRange) hMthTrkPhiDistvsRun->SetAxisRange(-0.2e-3, 0.6e-3, "Y");
...
Bool_t produceShade(TProfile *h, Int_t binLow, Int_t binHi, Double_t &mean, Double_t &rms, TGraphErrors *shade)
{
	...
	cout<<endl;
	cout<<"# of none-zero good variable statistic runs:"<<nRuns<<endl;
...
void addShade(TProfile *h, Int_t binLow, Int_t binHi, Int_t trgIdx)
{
	...
	Int_t nBadRuns   = 0;
	for(Int_t i=binLow;i<=binHi;i++){
		Int_t    nTrigEvts  = hnHT2EvtsvsRun->GetBinContent(i);
